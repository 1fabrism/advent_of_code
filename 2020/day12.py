...
with open("day12_input.txt", 'r') as file:
    ...
    input = [(line[:1], int(line[1:])) for line in input]
    directions = {"N":(0,1), "E":(1,0), "S":(0,-1), "W":(-1,0)}
    rotations = {"L":np.array([[0,1],[-1,0]]), "R":np.array([[0,-1],[1,0]])}
    ship_coords = (0,0)
    wp_coords = (10,1)
    for line in input:
        if line[0] in ['L', 'R']:
            n = line[1] // 90
            for i in range(n):
                wp_coords = np.dot(wp_coords, rotations[line[0]])
        elif line[0] == 'F':
            ship_coords += np.multiply(line[1], wp_coords)
        elif line[0] in directions.keys():
            wp_coords += np.multiply(line[1], directions[line[0]])
    print(f"Manhattan distance: {ship_coords[0]}E, {ship_coords[1]}N = {sum(abs(ship_coords))}")
