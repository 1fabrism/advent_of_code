...
with open("day16_input.txt",'r') as file:
    input = file.read()
    rules_raw, myticket, nearby_tickets = input.split('\n\n')
    ...
    for index,ticket in enumerate(nearby_tickets):
        ...
        nearby_tickets[index] = [int(n) for n in ticket_values]
    ...
    for rule in rules_raw:
        field_name, field_rule = rule.split(': ') 
        matches = reg.findall(field_rule)
        matches = [int(n) for n in matches]
        valid_range = list(range(matches[0],matches[1]+1)) + list(range(matches[2],matches[3]+1))
        rules[field_name] = valid_range
