...
with open("day13_input.txt",'r') as file:
    ...
    bus_IDs = [int(n) for n in input[1].split(',') if n.isdigit()]
    earliest_bus = max(bus_IDs)
    wait_minutes = earliest_bus
    for bus in bus_IDs:
        if (bus - timestamp % bus) < wait_minutes:
            earliest_bus = bus
            wait_minutes = bus - timestamp % bus
    print(f"Bus: {earliest_bus}, minutes: {wait_minutes}. Sum: {earliest_bus * wait_minutes}")
...
with open("day13_input.txt",'r') as file:
    ...
    while True:
        ...
        if any(mods):
            first_nonzero = [i for i, x in enumerate(mods) if x][0]
            for n in bus_IDs[0:first_nonzero]:
